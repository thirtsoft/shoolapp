...
import { ListeDepense } from '../../../../../../core/models/comptabilite/liste-depense';
import { ReferentielResourceService } from '../../../service/referentiel-resource.service';

@Component({
  selector: 'app-type-depense-component',
  standalone: true,
...
export class TypeDepenseComponent implements OnInit {
  ...
  errorMessage?: string;
  typeDepenseList: ListeDepense[] = [];
  typeDepenseId?: number;

  isEdit: boolean = true;
  isLoading: boolean = false;
  filteredDataDepense: any;
  ...
  ngOnInit(): void {
    this.chargerLesTypeDepenses();
  }

  async chargerLesTypeDepenses() {
    try {
      await Promise.all([
      ]);
  ...
  chargerLesDonnees(useFilterApi: boolean) {
    ...
    if (useFilterApi) {
      ...
      apiCall = this.refentielResource.getResourcePaged('typedepense', this.currentPage, this.pageSize);
    }
    apiCall.subscribe({
      next: (response) => {
        this.typeDepenseData = response.data?.content || [];
        this.totalElements = response.data?.totalElements || 0;
        this.columns = [
          { key: 'libelle', header: 'Libellé' },
        ];

        this.typeDepenseData = this.typeDepenseData.map((item: any) => ({
          ...item,
        }));
