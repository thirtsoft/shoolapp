...
export class CreateEditDepenseComponent implements OnInit {
  ...
  private getInvalidFields(): string[] {
    ...
    if (controls['designation']?.invalid) {
      champs.push('Désignation');
    }

    if (controls['typeDepense']?.invalid) {
      champs.push('Type dépense');
    }

    if (controls['montantDepense']?.invalid) {
      champs.push('Montant');
    }

    if (controls['modePaiement']?.invalid) {
      champs.push('Mode paiement');
    }
    ...
    this.depenseForm.get('modePaiement')?.valueChanges
      ...
      .subscribe(() => {
        ...
        if (this.shouldShowReference()) {
          ...
          referenceControl.setValue('', { emitEvent: false }
          );
        }
  ...
  private loadReferentialData(): void {
    ...
    this.referentielService.getResourceList('typedepense')
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(
            'Erreur',
            'Impossible de charger les types de dépense'
          );
        }
      });
    ...
    this.referentielService.getResourceList('moyenpaiement')
      ...
      .subscribe({
        ...
        error: (error) => {
          console.error(
            'Erreur chargement moyens paiement:',
            error
          );
  ...
  private loadDepense(depenseId: number): void {
    this.comptabiliteResource.recupererUneResource(this.RESOURCE_NAME, depenseId)
      ...
      .subscribe({
        next: (data: any) => {
          console.log('Dépense chargée pour modification :', data);
          console.log('Document V2 :', data?.documentResponse);
  ...
  private patchFormWithDepense(depense: Depense): void {
  ...
  getExistingFileName(): string {
    return (this.depense?.documentResponse?.nomFichier ?? 'Fichier');
  }
  ...
  removeExistingFile(): void {
    ...
    this.toastService.warning(
      'Attention',
      'La suppression de la pièce jointe existante nécessite l’API de suppression V2.'
    );
  }
  ...
  updateDepenseInfo(): void {
    ...
    if (this.depenseForm.invalid) {
      this.markFormGroupTouched(this.depenseForm);
      this.toastService.warning('Attention', 'Veuillez corriger les champs invalides');
      return;
    }
    ...
    this.comptabiliteResource.modifierUneRessource(this.RESOURCE_NAME, this.depenseId, payload)
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastService.error(
            'Erreur',
            error.error?.message || 'Erreur lors de la modification de la dépense'
          );
        }
      });
  }
  ...
  uploadFileOnly(): void {
    ...
    this.pieceJointeService.uploadUnePieceJointeObjet(this.RESOURCE_UPLOAD_FILE_NAME, this.depenseId, this.currentFile)
      .pipe(
        takeUntilDestroyed(this.destroyRef)
      )
      .subscribe({
  ...
  private buildPayload(): DepenseAddEdit {
    ...
    const request: DepenseAddEdit = {
      id: formValue.id,
      designation: formValue.designation?.trim(),
      typeDepense: formValue.typeDepense
        ? Number(formValue.typeDepense)
        : null,
      montantDepense: formValue.montantDepense
        ? Number(formValue.montantDepense)
        : null,
      modePaiement: formValue.modePaiement
        ? Number(formValue.modePaiement)
        : null,
  ...
  formatMontant(montant: number | null): string {
    ...
    const partieDecimale = parties.length > 1
      ? ',' + parties[1]
      : '';
  ...
  onMontantInput(event: Event): void {
    ...
    if (!isNaN(nombre)) {
      this.depenseForm.get('montantDepense')?.setValue(
        nombre,
        {
          emitEvent: false
        }
