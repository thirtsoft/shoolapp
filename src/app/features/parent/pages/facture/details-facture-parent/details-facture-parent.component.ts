...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, ReactiveFormsModule, DecimalPipe, CommonModule],
...
export class DetailsFactureParentComponent implements OnInit {
  ...
  getDetailsFacture(factureId: number) {
    this.comptabiliteResource.afficherDetailsResource('facture', factureId).subscribe({
      next: (data: any) => {
        ...
        console.log('Details facture {}', this.detailsFacture);
...

        {
          ...
          columns: [
            ...
            [
              ...
              {
                text: '+221 33 820 10 92 - BP 6268 Dakar étoile',
                fontSize: 10,
                alignment: 'left',
              },
              {
                text: 'www.ecolelesdauphins.org / ecolelesdauphin@gmail.com',
                fontSize: 10,
                alignment: 'left',
              },
              {
  ...
  goBack() {
    this.router.navigate(['parent/facture'])
  }
