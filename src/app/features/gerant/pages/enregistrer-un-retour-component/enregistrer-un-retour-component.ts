...
@Component({
  selector: 'app-enregistrer-un-retour-component',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './enregistrer-un-retour-component.html',
  styleUrl: './enregistrer-un-retour-component.css',
})
export class EnregistrerUnRetourComponent {

  readonly dataService = inject(DataService);

  sectionActive = signal<SectionType>('approvisionnement');
  sidebarCollapsed = signal(false);

  showCommandeLivreur = signal(true);
  showCommandeFournisseur = signal(false);
  ...
  onSidebarClose(): void {
    this.showCommandeFournisseur.set(false);
  }
  ...
  onChangeCommande(event: any) {
    console.log('checke', event);
    !this.showCommandeLivreur() && this.showCommandeFournisseur.set(true);
    if (this.showCommandeLivreur === signal(false)) {
      this.showCommandeFournisseur.set(true);
    }
    if (this.showCommandeFournisseur === signal(false)) {
      this.showCommandeLivreur.set(true);
    }
