...
@Component({
  ...
  templateUrl: './dashboard-gerant.html',
  styleUrl: './dashboard-gerant.css',
})
export class DashboardGerant {

  protected readonly Math = Math;
  ds = inject(DataService);

  vue = signal<Vue>('apercu');
  sidebarOpen = signal(false);
  sidebarCollapsed = signal(false);

  nav: { id: Vue; ico: string; label: string }[] = [
    { id: 'apercu', ico: '📊', label: 'Aperçu' },
    { id: 'livreurs', ico: '👷', label: 'Livreurs' },
    { id: 'livraisons', ico: '🚚', label: 'Livraisons' },
    { id: 'production', ico: '🏭', label: 'Production' },
    { id: 'ventes', ico: '💰', label: 'Ventes' },
  ];
  ...
  getLabel(id: Vue) { return this.nav.find(n => n.id === id)?.label ?? ''; }
