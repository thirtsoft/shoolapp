...
export class MonAgendaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getEnseignantByUtilisateur(this.userId);
  }


  async chargerLesCoursDeEnseignant() {
    try {
      await Promise.all([
        this.getClasseList(),
        this.getAnneesList()
  ...
  getEnseignantByUtilisateur(userId: number) {
    this.enseignantSerivce.getDetailsEnseignantUtilisateur(userId).subscribe({
      next: (data) => {
        ...
        this.chargerLesCoursDeEnseignant();
      },
      error: (err) => (err)
    });
  }
  ...
  chargerLesDonnees(useFilterApi: boolean) {
    ...
    if (useFilterApi) {
      ...
      apiCall = this.planificationResource.getResourceByIdPaged('planification/cours/enseignant', this.enseignantId!, this.currentPage, this.pageSize);
    } apiCall.subscribe({
      next: (response) => {
